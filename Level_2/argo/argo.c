...
/*   Updated: 2026/05/13 13:15:38 by yolim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	argo(json *dst, FILE *stream); // add function declaration
...
...: A list of pointers to variables where the read values will be stored.

fscanf(stream, "%d", &dst->integer) reads from a file pointed by the FILE pointer (ptr),
instead of reading from the input stream
Return Value: It returns zero or EOF, if unsuccessful.
Otherwise, it returns the number of items successfully assigned.

The "%d" conversion in fscanf requires an int * so the function knows where to store the parsed integer.
Passing &dst->integer gives fscanf the memory location to write into.
*/

int parse_integer(json *dst, FILE *stream) {
    dst->type = INTEGER;
    if (fscanf(stream, "%d", &dst->integer) != 1) // &dst->integer is the pointer to store the variable's value
        return (-1);
    return (1);
}
...
int parse_string(json *dst, FILE *stream) {
    ...
    if (!dst->string)   // dont forget to check
        return (-1);
    int j = 0;
...
int parse_map(json *dst, FILE *stream) {
    ...
    while (1) {
        ...
        if (argo(&current->value, stream) == -1)    // get pair's value
            return (-1);
...
int	argo(json *dst, FILE *stream) {
    ...
    c = peek(stream); // use peek, if not, charactor move to next one dy
    if (c == EOF) {
        unexpected(stream);
        return (-1);
    }
